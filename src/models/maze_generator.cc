...
void MazeGenerator::AddBottomWalls(Maze& maze, int row,
  ...
  for (size_t cell = 0; cell < static_cast<size_t>(maze.GetCols()); cell++) {
    ...
    size_t bottom_borders = std::count_if(
        sets_[cur_cell].begin(), sets_[cur_cell].end(), [&maze, row](int c) {
          return maze.GetMatrix()[row * maze.GetCols() + c] == BOTTOM_BORDER ||
    ...
    if (RandomChoice(bottom_wall_chance) && sets_[cur_cell].size() != 1 &&
      ...
      sets_[cur_cell].erase(
          std::find(sets_[cur_cell].begin(), sets_[cur_cell].end(), cell));
