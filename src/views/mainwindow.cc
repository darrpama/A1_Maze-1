...
MainWindow::MainWindow(QWidget* parent)
  ...
  connect(ui_->maze_init_random_btn, &QAbstractButton::clicked, this, &MainWindow::MazeInitRandomButtonClicked);

  connect(ui_->cave_upload_btn, &QAbstractButton::clicked, this, &MainWindow::CaveUploadButtonClicked);
  connect(ui_->cave_init_random_btn, &QAbstractButton::clicked, this, &MainWindow::CaveInitRandomButtonClicked);

  connect(ui_->cave_step_render_btn, &QAbstractButton::clicked, this, &MainWindow::CaveStepRenderButtonClicked);
  connect(ui_->cave_auto_render_btn, &QAbstractButton::clicked, this, &MainWindow::CaveAutoRenderButtonClicked);
  connect(ui_->cave_stop_render_btn, &QAbstractButton::clicked, this, &MainWindow::CaveStopRenderButtonClicked);
}
...
void MainWindow::MazeUploadButtonClicked() {
  ...
  if (!file_path.isEmpty()) {
    try {
      ...
      ui_->maze_rows_input->setValue(s21::ControllerSingleton::GetInstance().GetMazeRows());
      ui_->maze_cols_input->setValue(s21::ControllerSingleton::GetInstance().GetMazeCols());
    } catch(const std::exception& e) {
      ShowErrorMessage(e);
    }
  }
...
void MainWindow::MazeDownloadButtonClicked() {
    QString file_path = QFileDialog::getSaveFileName(
      this, tr("Select File"), "", tr("TXT (*.txt)")
    );

    try {
        s21::ControllerSingleton::GetInstance().DownloadMaze(file_path.toStdString());
...
void MainWindow::CaveUploadButtonClicked() {
  ...
  if (!file_path.isEmpty()) {
    try {
      ...
      ui_->cave_rows->setValue(s21::ControllerSingleton::GetInstance().GetCaveRows());
      ui_->cave_cols->setValue(s21::ControllerSingleton::GetInstance().GetCaveCols());
    } catch (const std::exception& e) {
      ShowErrorMessage(e);
    }
  }
